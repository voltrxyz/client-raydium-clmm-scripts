...
const decreaseRaydiumCLMMLiquidity = async () => {
  ...
  for (let i = 0; i < poolInfo.rewardDefaultInfos.length; i++) {
    ...
    const rewardMint = new PublicKey(
      poolInfo.rewardDefaultInfos[i].mint.address
    );
    const rewardProgram = new PublicKey(
      poolInfo.rewardDefaultInfos[i].mint.programId
    );
  ...
  const { additionalArgs, addressLookupTableAccounts } =
    await setupJupiterSwapForWithdrawStrategy(
      connection,
      decreaseLiquidityAmount.mul(new BN(10_000)).div(new BN(10_000 * 2 + 10)),
      payer,
      vaultStrategyAuth,
      Buffer.from([]),
      remainingAccounts,
