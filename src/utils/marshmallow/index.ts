...
export class WideBits<P extends string = ""> extends Layout<
...
export function publicKey<P extends string = "">(
  property?: P
): Layout<PublicKey, P> {
...
export function option<T, P extends string = "">(
  layout: Layout<T>,
  property?: P
): Layout<T | null, P> {
...
export function vec<T, P extends string = "">(
...
export function tagged<T, P extends string = "">(
  tag: BN,
  layout: Layout<T>,
  property?: P
): Layout<T, P> {
...
export function vecU8<P extends string = "">(property?: P): Layout<Buffer, P> {
  ...
  const layout: Layout<{ data: Buffer }> = struct([
    length,
    blob(_offset(length, -length.span), "data"),
  ]) as any; // Something I don't know
  return new WrappedLayout(
    layout,
    ({ data }) => data,
...
export function rustEnum<T, P extends string = "">(
  variants: Layout<any>[],
  property?: P
): EnumLayout<T, P> {
...
export function array<T, P extends string = "">(
  ...
  ]) as any as Layout<{ values: T[] }>; // Something I don't know
  return new WrappedLayout(
    layout,
    ({ values }) => values,
    (values) => ({ values }),
...
export type GetStructureFromLayoutSchema<S extends { [key: string]: any }> =
  Structure<any, any, S>;

export class Union<
  Schema extends { [key: string]: any }
> extends _Union<Schema> {
  encodeInstruction(instruction: any): Buffer {
    const instructionMaxSpan = Math.max(
      ...Object.values(this.registry).map((r) => r.span)
    );
...
export function seq<T, P extends string = "", AnotherP extends string = "">(
  ...
  const superCount =
    typeof count === "number"
      ...
      ? count.toNumber()
      : new Proxy(
          count as unknown as Layout<number> /* pretend to be Layout<number> */,
          {
            get(target, property): any {
              if (!parsedCount) {
                ...
                parsedCount = isBN(countProperty)
                  ? countProperty.toNumber()
                  : countProperty;
